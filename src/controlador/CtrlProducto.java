...
public class CtrlProducto extends CtrlImprimir implements ActionListener, CaretListener, MouseListener, KeyListener {
	...
	this.menu.tblProductos.addMouseListener(this);
        this.id = null;
...
//        menu.btnCalcularGanancia.setVisible(false);
    ...
    public void MostrarProductos(String buscar) {
        ...
        if(this.permiso == 1){
            menu.tblProductos.setModel(this.productos.ConsultaAdmin(buscar));
	...
	menu.txtAreaComposicionProducto.setText("");
	menu.txtAreaComposicionProducto.setText("");
	...
	menu.txtAreaComposicionProducto.setEnabled(true);
...
//este metodo iversion es el mismo metodo que esta en reportes lo repite para no crear una nueva instancia de la clase reportes
    ...
    public void editarProducto() {
		...
		composicion = (String) modelo.getValueAt(filaseleccionada,15);
		precioCompraDolar = Float.parseFloat((String) modelo.getValueAt(filaseleccionada, 16).toString());
		...
		menu.txtAreaComposicionProducto.setText(composicion);
		menu.jsPrecioCompraDolar.setValue(precioCompraDolar);
                this.id = id;
                menu.btnGuardarProducto.setEnabled(false);
    ...
    public void guardarProducto() {
		...
		composicion = menu.txtAreaComposicionProducto.getText();
                float utilidad = Float.parseFloat(menu.spUtilidad.getValue().toString()),
			precioCompraDolar = Float.parseFloat(menu.jsPrecioCompraDolar.getValue().toString());
    ...
    public void actualizarProducto() {
		...
		composicion = this.menu.txtAreaComposicionProducto.getText();
        float utilidad = Float.parseFloat(menu.spUtilidad.getValue().toString()),
		precioCompraDolar = Float.parseFloat(this.menu.jsPrecioCompraDolar.getValue().toString());
        Date fechaVencimiento = menu.jcFechaVProducto.getDate();
        long fechaV = fechaVencimiento.getTime();
        java.sql.Date fecha = new java.sql.Date(fechaV);
        if (nombre.equals("")) {
	...
	if(e.getSource() == menu.tblProductos)
	{
		String id;
		int filaseleccionada = menu.tblProductos.getSelectedRow();
		...
		try {
			if(filaseleccionada != -1)
			{
				if(click == 2 && this.permiso == 1)
				{
					...
					this.menu.lblTitleInfoProducto.setText("Información de "+ info.get(2));
					this.menu.lblIdProducto.setText(info.get(0));
					this.menu.lblCodBarraProducto.setText(info.get(1));
					this.menu.lblNombreProducto.setText(info.get(2));
					this.menu.lblPrecioCompraProducto.setText(info.get(3));
					this.menu.lblMonedaCompraProducto.setText(info.get(4));
					this.menu.lblPrecioCompraDolarProducto.setText(info.get(5));
					this.menu.lblPrecioVentaProducto.setText(info.get(6));
					this.menu.lblMonedaVentaProducto.setText(info.get(7));
					this.menu.lblPrecioMinProducto.setText(info.get(8));
					this.menu.lblMarcaProducto.setText(info.get(9));
					this.menu.lblCategoriaProducto.setText(info.get(10));
					this.menu.lblDescripcionProductoInfo.setText(info.get(11));
					this.menu.lblUtilidadProducto.setText(info.get(12));
					this.menu.lblComposicionProducto.setText(info.get(13));
					this.menu.lblFechaVencimientoProducto.setText(info.get(14));
					this.menu.lblStockProducto.setText(info.get(15));
					this.menu.lblUbicacionProducto.setText(info.get(16));
					this.menu.jdInfoProductos.setLocationRelativeTo(null);
					this.menu.jdInfoProductos.setSize(1014, 580);
					this.menu.jdInfoProductos.setVisible(true);
				}
			}
		} catch (Exception err) {
		}
    ...
    public void DeshabilitarProductos() {
        ...
        menu.spPrecioMinimoProducto.setEnabled(false);
	menu.txtAreaComposicionProducto.setEnabled(false);
